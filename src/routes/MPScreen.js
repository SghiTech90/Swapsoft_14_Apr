...
const MPScreen = ({navigation, route}) => {
  ...
  const allNutritionData = {
    Building: [
      {label: 'Total', value: 30, max: 20},
      {label: 'Completed', value: 2, max: 20},
      {label: 'In Progress', value: 5, max: 20},
      {label: 'Pending', value: 10, max: 20},
    ],
    Road: [
          ...
          <ScrollView contentContainerStyle={styles.scrollViewContent}>
            ...
            <View style={styles.headWiseContainer}>
              ...
              {headWiseData?.map((item, index) => (
                <View key={index}>
                  {/* Row with plus/minus icon and title */}
